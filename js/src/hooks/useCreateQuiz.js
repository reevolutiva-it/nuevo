...
const useGetListQuiz = (course_id, callback=false) => {
    wp.apiRequest({
        method:'GET',
        path:'/ldlms/v2/sfwd-quiz?course='+course_id,
...
const useCreateQuiz = (props, callback = false) => {
    wp.apiRequest({
        method:'POST',
        path:'/ldlms/v2/sfwd-quiz',
        data: props
    }).then(e => {
        if(callback != false){
            callback(e);
...
const useUpdateQuiz = (props, id, callback = false) => {
    wp.apiRequest({
        method:'POST',
        path:'/ldlms/v2/sfwd-quiz'+id,
        data:props
    }).then(e => {
        if(callback != false){
            callback(e);
...
export { useCreateQuiz , useUpdateQuiz, useGetListQuiz };